...
- **MCP `ToolAnnotations` on the 10 tools.** Initially included in this release, but adding them caused the `tests/test_subprocess_launch.py` regression test (which sends an MCP `tools/list` request to the subprocess server) to never receive a response — server starts and processes `initialize`, but `tools/list` silently drops. Same failure on Windows and Linux CI. Root cause not yet identified (could be a FastMCP 2.x serialization issue with tools that carry annotations, or a race condition in the test's stdio handling that the annotations changes shifted into the failure regime). Deferred to a follow-up release where we can investigate properly.
